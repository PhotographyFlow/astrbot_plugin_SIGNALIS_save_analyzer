...
def decode_mod_from_png(path: Path) -> ModSaveData:
    ...
    try:
        ...
        raise TranslationError(
            ErrorKind.ImageReadError, f"Failed to open image at {path}: {e}"
        )
    ...
    try:
        ...
        raise TranslationError(
            ErrorKind.ImageDecodeError, f"Decoded data is not valid UTF-8: {e}"
        )
    except json.JSONDecodeError as e:
        raise TranslationError(
            ErrorKind.ImageDecodeError, f"Decoded data is not valid JSON: {e}"
        )
    ...
    if len(vector_keys) != len(vectors_raw):
        raise TranslationError(
            ErrorKind.ImageDecodeError, "Mismatch between vectorKeys and vectors count"
        )
    vectors = {k: [v["x"], v["y"], v["z"]] for k, v in zip(vector_keys, vectors_raw)}

    int_keys = json_dict.get("intKeys", [])
    ints = json_dict.get("ints", [])
    if len(int_keys) != len(ints):
        raise TranslationError(
            ErrorKind.ImageDecodeError, "Mismatch between intKeys and ints count"
        )
    ...
    if len(float_keys) != len(floats):
        raise TranslationError(
            ErrorKind.ImageDecodeError, "Mismatch between floatKeys and floats count"
        )
    ...
    if len(bool_keys) != len(bools):
        raise TranslationError(
            ErrorKind.ImageDecodeError, "Mismatch between boolKeys and bools count"
        )
    ...
    if len(string_keys) != len(strings):
        raise TranslationError(
            ErrorKind.ImageDecodeError, "Mismatch between stringKeys and strings count"
        )
